...
export default function ChatComposer({ onSend, sending, replyTo, onCancelReply }) {
  ...
  return (
    <div className="px-3 pt-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] sm:pb-3 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 shrink-0">
      {replyTo && (
      ...
      <form onSubmit={handleSubmit} className="flex items-end gap-2">
        <div className="flex items-center mb-1">
          <button
            type="button"
            className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-blue-500 transition-colors touch-manipulation"
          >
            <Paperclip size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 relative bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-transparent transition-all">
          ...
          <button
            type="button"
            className="absolute right-2 bottom-1.5 p-1.5 text-slate-400 hover:text-amber-500 transition-colors touch-manipulation"
          >
            <Smile size={20} />
          </button>
        </div>
