...
// Tính ngày hôm nay một lần – dùng lại trong module scope
const getTodayStr = () => {
  const t = new Date();
...
export const StatusBadge = memo(function StatusBadge({ status, dueDate, evaluationScore }) {
  ...
  const info = STATUS_MAP[effective] || STATUS_MAP.pending;
  return (
...
export const ScoreBadge = memo(function ScoreBadge({ score, rank, showScore = true }) {
  return (
    <div className="flex flex-col items-center gap-1">
      {showScore && <span className="text-[13px] font-black text-blue-600 dark:text-blue-400">{score}</span>}
      {rank && (
        <span className={`px-1.5 py-0.5 text-[9px] font-black rounded-md whitespace-nowrap ${RANK_COLOR[rank] || ''}`}>
          {rank}
        </span>
      )}
    </div>
...
export const EvaluationStatusBadge = memo(function EvaluationStatusBadge({ task, showScore = true }) {
  ...
  const info = EVAL_STATUS_MAP[evalStatus] || EVAL_STATUS_MAP.waiting_self;
